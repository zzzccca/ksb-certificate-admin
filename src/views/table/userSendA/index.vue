<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      ...
      <el-radio  v-model="radio" label="待寄">待寄</el-radio>
      <el-radio  v-model="radio" label="已寄">已寄</el-radio>
      <el-radio  v-model="radio" label="已收">已收</el-radio>
      <el-radio v-model="radio" label="">全部</el-radio>
      <el-button class="import" style="margin-left: 20px" type="primary" >
        <i class="el-icon-upload el-icon--right"></i>  导入单号
        <form id="tf">
          <input class="import-input" type="file" name="filename" @click="onEmptyClick" @change="onImportChange"/>

        </form>
      </el-button>
      <el-button  type="primary" icon="document" @click="onExportUserClick">导出待寄</el-button>
    </div>
    <el-table :data="list" style="width: 100%" v-loading.body="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row>
      ...
      <el-table-column label="证书名"  width="200" align="center">
        <template scope="scope">
      ...
      <el-table-column label="申请人姓名" width="150" align="center">
        <template scope="scope">
          {{scope.row.name}}
        </template>
      </el-table-column>
...
<script>
  import {getList,upload,reject} from '@/api/usersenda';
  ...
  export default {
    ...
    methods: {
      ...
      onExportUserClick(){ //导出
        location.href='http://www.embracex.com:8081/getexpressageexl';
      },
